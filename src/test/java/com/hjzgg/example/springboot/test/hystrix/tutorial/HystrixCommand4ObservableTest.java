 ...
 * <p>
 * 2）observe()中，toBlocking().single()和subscribe()可以共存；在toObservable()中不行，
 * 因为两者都会触发执行HelloWorldHystrixCommand.run()，这违反了同一个HelloWorldHystrixCommand对象只能执行run()一次原则
 *
 * @throws Exception
 */
public class HystrixCommand4ObservableTest {
    ...
    public void testObservable() throws Exception {
        ...
        // single()是堵塞的
        /**
         * 会立马去执行run 执行run之后再执行onNext方法-> onError(), 异常则触发 onError()
         */
        System.out.println("hotObservable single结果：" + hotObservable.toBlocking().single());

        // 注册观察者事件
        // subscribe()是非堵塞的
//		是从observe()返回后调用程序调用subscribe()完成事件注册
        hotObservable.subscribe(new Observer<String>() {
            ...
            /**
             * retrieve the value of the command by subscribing to the Observable
             */
            @Override
        ...
        // use java8
        hotObservable.subscribe(v -> System.out.println("java8 onNext: " + v));

...
import com.hjzgg.example.springboot.hystrix.HelloWorldHystrixObservableCommand;
 ...
 * 1）observe()会立即执行HelloWorldHystrixObservableCommand.construct()；
 *    toObservable()要在toBlocking().single()或subscribe(), 时才执行HelloWorldHystrixObservableCommand.construct()
 * 2）observe()中，toBlocking().single()和subscribe()可以共存；
 *    在toObservable()中不行，因为两者都会触发执行HelloWorldHystrixObservableCommand.construct()，
 *    这违反了同一个HelloWorldHystrixObservableCommand对象只能执行construct()一次原则
 * @throws Exception
 */
...
//		System.out.println("11111111");
		Observable<String> hotObservable = new HelloWorldHystrixObservableCommand("Hlx").observe();   // 立即执行run or construct, hot
...
//		System.out.println("11111111");
		Observable<String> coldObservable = new HelloWorldHystrixObservableCommand("Hlx").toObservable();   // 不会立马执行run() construct() cold
//		TimeUnit.MILLISECONDS.sleep(2000);
//		System.out.println("22222222");
